...
.controller('speciesController', function($scope, $sce, speciesFactory) {
    ...
    var species = ["Mammals","Birds","Reptiles","Amphibians","Fish"];
    ...
    function getSpecies(args, $scope, $sce) {
        ...
        for (var speciesIdx = 0; speciesIdx < species.length; speciesIdx++) {
           makeApiCall(speciesIdx, parkID, xml2json, $scope, $sce);
        }
    }

    function makeApiCall(speciesIdx, parkID, xml2json, $scope, $sce) {
        var url = 'https://irmaservices.nps.gov/v3/rest/npspecies/fulllist/' + parkID + "/" + species[speciesIdx];
        var yql = 'https://query.yahooapis.com/v1/public/yql?q=' + encodeURIComponent('select * from xml where url="' + url + '"') + '&format=xml&callback=?';
         ...
         $.ajax({
                ...
                success: function(data) {
                    ...
                    var speciesName = species[speciesIdx];

                    $scope.$apply(function() {
                        $scope.textArea = $sce.trustAsHtml($scope.textArea + "<div class='speciesHolder'><h4 class='speciesTitle'>" + species[speciesIdx] + "</h4><div species = '" + speciesName + "'  class='AllAnimals'>" + others + "</div></div>");
                    });
                },
